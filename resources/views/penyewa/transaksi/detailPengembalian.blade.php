            ...
            <div class="container">
                <div class="row">
                    <div class="col-3">
                        ...
                        <img src="{{ asset($order->produk->getFotoProdukFirst($order->produk->id)->path) }}" class="img-thumbnail" alt="fotoproduk">
                    ...
                    <div class="col-6">
                        ...
                        <form action="">
                            ...
                            <h1><strong>{{ $nama_produk }}</strong></h1>
                            ...
                            <div class="namapenyewa-ctner mt-3">
                                ...
                                <input type="text" name="nama" placeholder="Nama Penyewa" class="form-control form-control-lg w-100" value="{{ $nama_penyewa }}" readonly />
                            ...
                            <div class="noresi-ctner mt-3">
                                ...
                                <input type="text" name="nomorresi" placeholder="Nomor Resi" class="form-control form-control-lg w-100" value="{{ $order->nomor_resi }}" readonly />
                                <div id="emailHelp" class="form-text">Nomor resi pengiriman untuk pelacakan. klik <span><a href="https://www.cekresi.com"
                                            target="_blank">disini</a></span> untuk mengecek
                                    resi</div>
                            </div>
                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <table class="w-100">
                                    ...
                                    <tr class="text-secondary">
                                        <td>Harga Katalog</td>
                                        <td id="harga-katalog" class="text-end">Rp{{ number_format($harga_katalog, 0, '', '.') }}</td>
                                    </tr>
                                    <tr class="text-secondary">
                                        <td>Harga Additional</td>
                                        @if ($order->additional)
                                            @foreach ($order->additional as $additionalItem)
                                                <td id="harga-additional-total" class="text-end">Rp{{ number_format($additionalItem['harga'], 0, '', '.') }}</td>
                                            @endforeach
                                        @else
                                    ...
                                    <tr class="text-secondary">
                                        <td>Harga Cuci</td>
                                        <td id="harga-cuci" class="text-end">Rp{{ number_format($order->biaya_cuci ?? 0, 0, '', '.') }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr class="text-secondary">
                                        ...
                                        <td id="biaya-admin-value" class="text-end">Rp{{ number_format($order->fee_admin, 0, '', '.') }}</td>
                                    </tr>
                                </table>
                                ...
                                <table class="w-100">
                                    ...
                                    <tr class="text-secondary">
                                        <td>Ongkos Kirim</td>
                                        <td id="harga-ongkir-pengiriman" class="text-end">Rp{{ number_format($order->ongkir_pengiriman ?? 0, 0, '', '.') }}</td>
                                    </tr>
                                    <tr class="text-secondary">
                                        <td>Denda Terlambat</td>
                                        <td id="harga-denda-keterlambatan" class="text-end">Rp{{ number_format($order->denda_keterlambatan ?? 0, 0, '', '.') }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr class="text-secondary">
                                        <td>Jaminan Ongkir</td>
                                        <td id="ongkos-kirim" class="text-end">Rp{{ number_format($order->ongkir_default ?? 0, 0, '', '.') }}</td>
                                    </tr>
                                    <tr class="text-secondary">
                                        <td>Jaminan Kostum</td>
                                        @if ($order->jaminan < 0)
                                            <td id="jaminan-kostum" class="text-end">Rp{{ number_format(0) }}</td>
                                        @else
                                            <td id="jaminan-kostum" class="text-end">Rp{{ number_format($order->jaminan ?? 0, 0, '', '.') }}</td>
                                        @endif
                                    </tr>
                                </table>
...
<!-- 
                                ...
                                @if ($order->jaminan >= 0)
                                    <h5 class="mt-2"><strong>Total Uang Kembali <i class="ri-information-line" style="cursor: pointer" data-bs-toggle="modal"
                                                data-bs-target="#strukBelanjaModal"></i></strong></h5>
                                    <h4><strong id="total-tagihan">Rp{{ number_format(abs($uangKembali), 0, '', '.') }}</strong>
                                    </h4>
                                @elseif ($order->jaminan < 0)
                                    <h5 class="mt-2 text-danger"><strong>Total Hutang <i class="ri-information-line" style="cursor: pointer" data-bs-toggle="modal"
                                                data-bs-target="#strukBelanjaModal"></i></strong>
                                    </h5>
                                    <h4 class="text-danger"><strong id="total-tagihan">Rp{{ number_format(abs($order->jaminan), 0, '', '.') }}</strong>
                                    </h4>
                                @endif
        ...
        <div class="modal fade" id="strukBelanjaModal" tabindex="-1" aria-labelledby="strukBelanjaModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-body">
                        <table class="w-100 mb-3">
                            ...
                            <tr>
                                ...
                                @if ($order->additional)
                                    @foreach ($order->additional as $additionalItem)
                                        <td class="text-end">Rp{{ number_format($additionalItem['harga'], 0, '', '.') }}</td>
                                    @endforeach
                                @else
                            ...
                            <tr class="text-danger">
                                ...
                                <td class="text-end">:</td>
                                <td class="text-end">Rp{{ number_format($order->ongkir_pengiriman ?? 0, 0, '', '.') }}</td>
                            </tr>
                            <tr class="text-danger">
    ...
    <script type="text/javascript">
        document.getElementById('pay-button').onclick = function() {
            $.ajax({
                url: '{{ route('createSnapTokenDenda', $order->nomor_order) }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                },
                    ...
                    if (response.success) {
                        ...
                        snap.pay(response.snap, {
                            onSuccess: function(result) {
                                $.ajax({
                                    url: '{{ route('updatePenghasilan', $order->nomor_order) }}',
                                    type: 'POST',
                                    data: {
                                        _token: '{{ csrf_token() }}',
                                    },
