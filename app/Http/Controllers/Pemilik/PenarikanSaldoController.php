...
class PenarikanSaldoController extends Controller
{
    ...
    public function viewTarikSaldo()
    {
        ...
        if (!$rekening || !$rekening->saldo || $rekening->saldo = 0) {
            return redirect()->route('seller.keuangan.dashboardKeuangan')->with('error', 'Anda tidak memiliki saldo yang dapat ditarik / Rekening belum ditetapkan. Tidak dapat menarik saldo');
        } else if (!$rekening->nomor_rekening) {
            return redirect()->route('seller.keuangan.dashboardKeuangan')->with('error', 'Anda belum menetapkan nomor rekening anda. Klik icon pensil');
        }
